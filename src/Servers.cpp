        ...
        generic_async_to_bitcoind(c, batchId, reqId, "sendrawtransaction", params,
            ...
            [size = rawtxhex.length() / 2, c, this, txkey](const RPC::Message &reply) {
                ...
                QByteArray logLine;
                {
                    ...
                    if (!options->anonLogs)
                        ts << ", size: " << size << " bytes, response: " << ret.toString();
                }
             ...
             timeoutSecs = options->slipstreamTimeoutSecs] {
                {
                    ...
                    res->feeKnown = feeOpt->feeKnown;
                    if (!res->feeKnown)
                        Log() << "Slipstream fee unknown for txid " << res->txid
